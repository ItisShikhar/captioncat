...
/** Underline decoration under a word. */
export class Underline extends Component {
  ...
  override getMargins(_ctx: ResolveContext, source?: EffectSource): Margins {
    ...
    let margins = { x: protrusion + offset.x, y: width + offset.y };
  ...
  /** 'behind' (default) or 'inFront' of the glyph fill. */
  renderOrder(rctx: ResolveContext): 'behind' | 'inFront' {
    const raw = String(this.getProp<string>('renderOrder')?.resolve(rctx) ?? '')
      .trim()
      .toLowerCase();
    return raw === 'infront' ? 'inFront' : 'behind';
  }

  /** Draw the underline beneath the glyphs at the origin. */
  paintLine(
    ctx: CanvasContext2D,
    text: string,
    fontPaint: Paint,
    ...
    const offset =
      rawOffset == null ? { x: 0, y: Math.max(1, Math.round(Math.max(descent * 0.7, 2))) } : toVec2(rawOffset);
    const width = Number(
      this.getProp<number>('width')?.resolve(rctx) ?? Math.max(1, Math.round(Math.max(ascent + descent, 12) * 0.08)),
    );
    const lineY = descent + offset.y;
